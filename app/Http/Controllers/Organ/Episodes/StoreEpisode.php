...
use App\Http\Controllers\Organ\Episodes\IndexEpisode;
...
use App\Rules\OrganChapterRule;
...
class StoreEpisode extends BaseComponent
{
    ...
    public function mount($action , $id = null)
    {
        ...
        if ($this->mode == self::UPDATE_MODE) {
            $this->episode = $this->episodeRepository->findOrganEpisode($id);
    ...
    public function render()
    {
        ...
        return view('organ.episodes.store-episode',get_defined_vars())
    ...
    public function store()
    {
        if ($this->mode == self::UPDATE_MODE) {
            if (is_null($this->episode->transcripts) || $this->episode->transcripts->where('status',EpisodeEnum::PENDING_STATUS)->count() == 0) {
                ...
                redirect()->route('organ.episodes',['tab'=>IndexEpisode::TRANSCRIPTS]);
            } else
    ...
    {
        if (collect(Auth::user()->organCourses)->contains('id',$value))
