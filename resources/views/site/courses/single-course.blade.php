                                            ...
                                            <div class="card-header" id="heading{{$item['id']}}">
                                                <button class="btn btn-link d-flex align-items-center justify-content-between" data-toggle="collapse" data-target="#collapse{{$item->id}}" aria-expanded="true" aria-controls="collapse{{$item->id}}">
                                                    ...
                                                    @elseif(auth()->check() && $user->hasCourse($course->id))
                                                        <small class="text-success">خریداری شده <i class="la la-lock-open"> </i></small>
                                                    @elseif(!auth()->check() || (auth()->check() && !$user->hasCourse($course->id)))
                                                        <div class="text-left">
                                                            <small class="text-danger d-block">نقدی <i class="la la-lock"> </i></small>
                                                        </div>
                                                    @endif
                                                </button>
                                            </div>
                            ...
                            <form method="post" id="commentForm" class="row" wire:submit.prevent="new_comment">
                                ...
                                <div class="input-box col-lg-12">
                                    ...
                                    <div class="form-group">
                                        <textarea wire:model.defer="comment" class="form-control form--control pl-3" name="message" placeholder="پیام بنویس" rows="5"></textarea>
                                    </div>
                                    @error('comment')
                        ...
                        <div class="course-overview-card pt-4">
                                        ...
                                        <div class="media media-card  pb-4 mb-1">
                                            ...
                                            <div class="media-body">
                                                ...
                                                <div class="helpful-action">
                                                    <button wire:click="$set('actionComment',{{$comments[$i]->id}})" class="btn btn-outline-success goToCommentForm">پاسخ</button>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div class="see-more-review-btn text-center">
                                @if($commentCount < $comments->count())
                                    <button type="button" wire:click="moreComment()" class="btn theme-btn theme-btn-transparent">بارگیری نظرات بیشتر</button>
                                @endif
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="card card-item">
                            <div class="card-body">
                                ...
                                <ul class="generic-list-item generic-list-item-flash">
                                    ...
                                    <li class="d-flex align-items-center justify-content-between">
                                        <span><i class="la la-certificate mr-2 text-color"></i>گواهی</span> {{ (!is_null($course->quiz) && !is_null($course->quiz->certificate)) ? 'بعله' : 'خیر' }}
                                    </li>
                                </ul>
                            </div>
                        </div>
                            ...
                            <div class="card-body">
                                ...
                                @foreach($related_courses as $item)
                                    ...
                                    <div class="media-body">
                                        ...
                                        <p class="text-black font-weight-semi-bold lh-18 fs-15">{{ $item->price >0 ? number_format($item->price).' تومان' : 'رایگان' }}</p>
                                    </div>
                                </div>
                                @endforeach
                                <!-- end media -->
                                <div class="view-all-course-btn-box">
                                    <a href="{{ route('courses') }}" class="btn theme-btn w-100">مشاهده همه دوره ها <i class="la la-arrow-left icon ml-1"></i></a>
                                </div>
                            </div>
                        </div>
                        <!-- end card -->
