<div class="container-fluid m-0 p-0 p-2">
                            ...
                            <td colspan="12" class="hiddenRow">
                                <div wire:ignore.self class="accordian-body collapse" id="episodes{{$key}}">
                                    <fieldset class="border p-2">
                                        ...
                                        @if(! $has_file)
                                            <button wire:click="$set('has_file','true')" class="btn btn-sm btn-success">افزودن فایل <i class="fa fa-file"></i></button>
                                        @endif
                                        @if(! $has_link)
                                            <button wire:click="$set('has_link','true')" class="btn btn-sm btn-info">افزودن لینک <i class="fa fa-link"></i></button>
                                        @endif
                                        @if(! $has_video)
                                            <button wire:click="$set('has_video','true')" class="btn btn-sm btn-warning">افزودن ویدیو <i class="fa fa-video"></i></button>
                                        @endif
                                        @if(! $has_homework)
                                            <button wire:click="$set('has_homework','true')" class="btn btn-sm btn-danger">افزودن تمرین <i class="fa fa-file-archive"></i></button>
                                        @endif
                                        @if(! $has_bucket)
                                        ...
                                        <div class="row mt-4">
                                            ...
                                            <x-admin.forms.lfm-standalone hidden="{{ ! $has_file }}" with="6" id="new_file" label="فایل" :file="$new_file" type="image"
                                                                          required="true" wire:model="new_file" />

                                            <x-admin.forms.input hidden="{{ ! $has_link }}" type="url" id="new_link{{$key}}" label="لینک" wire:model.defer="link" />

                                            <x-admin.forms.input hidden="{{ ! $has_video }}" with="2" type="text" id="new_time{{$key}}" label="زمان *" wire:model.defer="time" />
                                            <x-admin.forms.dropdown hidden="{{ ! $has_video }}" with="4" id="new_video_storage{{$key}}" :data="$data['storage']"
                                                                    label=" فضای ذخیره سازی ویدئو" wire:model.defer="video_storage" />

                                            <x-admin.forms.lfm-standalone hidden="{{ ! $has_video }}" with="6" id="new_local_video" label="ویدئو" :file="$new_local_video"
                                                                          type="image" required="true" wire:model="new_local_video" />

                                            <x-admin.forms.checkbox hidden="{{ ! $has_video }}" with="2" value="1" id="new_allow_show_local_video{{$key}}"
                                                                    label="امکان نمایش ویدئو " wire:model.defer="allow_show_local_video" />
                                            <x-admin.forms.checkbox hidden="{{ ! $has_video }}" with="2" value="1" id="new_downloadable_local_video{{$key}}"
                                                                    label="امکان دانلود ویدئو " wire:model.defer="downloadable_local_video" />
                                            <x-admin.forms.checkbox hidden="{{ ! $has_video }}" with="2" value="1" id="new_show_api_video{{$key}}"
                                                                    label="نمایش ویدئو " wire:model.defer="show_api_video" />

                                            <x-admin.forms.text-area hidden="{{ ! $has_bucket }}" label="کد اشتراک گذاریapi " wire:model.defer="api_bucket" id="new_api_bucket{{$key}}" />

                                            <x-admin.forms.checkbox hidden="{{ ! $has_homework }}" with="2" value="1" id="new_can_homework{{$key}}" label="امکان بارگذاری تمرین "
    ...
    <x-admin.modal-page id="episode" title="{{$title}}" wire:click="saveEpisode('{{$epiosde_key}}')">
        ...
        <div class="row">
            ...
            <x-admin.forms.full-text-editor id="description2" label="توضیحات" wire:model.defer="description"/>
