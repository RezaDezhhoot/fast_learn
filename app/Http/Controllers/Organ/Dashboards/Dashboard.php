...
namespace App\Http\Controllers\Organ\Dashboards;
...
class Dashboard extends BaseComponent
{
    ...
    public $to_date_viwe , $from_date_view , $teachers = [] , $organs = [];
    ...
    public function mount()
    {
        ...
        $this->box = [
            'courses' => $this->courseRepository->getOrgansCount($this->from_date,$this->to_date),
            'episodes' => $this->episodeRepository->getOrganCount($this->from_date,$this->to_date),
            'pending_course' => $this->newCoursesRepository->getOrgansCount(),
            'students' =>  $this->orderDetailRepository->getOrgansStudents($this->from_date,$this->to_date),
        ];
    }
    ...
    public function confirmFilter()
    {
        ...
        redirect()->route('organ.dashboard',
            [
                'from_date'=> $from_date,
                'to_date'=> $to_date,
            ]
        ...
        foreach ($chartModels as $chartModel ) {
            ...
            for ($i = 0 ; $i< count($dates); $i++) {
                $chart[$chartModel][] = (float)$this->orderDetailRepository->getDashboardDataPayments(
                    $dates[$i]->format('Y-m-d') . " 00:00:00", $dates[$i]->format('Y-m-d') . " 23:59:59" , 'organ_amount',
                    organ: true
                );
    ...
    {
        return view('organ.dashboards.dashboard')->extends('organ.layouts.organ');
    }
}
